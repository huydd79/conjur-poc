...
PUB_CRT_FILE="data/jwtgen/jwt.pub"

cp -r policies data/

# Loading conjur policy for jwt authentication
echo "Configuring authn-jwt/$JWT_SERVICE_ID authenticator... "
sed -i "s/\$JWT_SERVICE_ID/$JWT_SERVICE_ID/g" data/policies/policy-jwt-auth.yaml
conjur policy load -b root -f data/policies/policy-jwt-auth.yaml
[[ $? -eq 0 ]] && echo "Done!!!"  || echo "ERROR!!!"

# Loading conjur policy for host id and its permission
echo "Loading policy for $JWT_HOST_ID and its permission... "
sed -i "s/\$JWT_SERVICE_ID/$JWT_SERVICE_ID/g" data/policies/policy-app-access.yaml
sed -i "s/\$JWT_HOST_ID/$JWT_HOST_ID/g" data/policies/policy-app-access.yaml
conjur policy load -b root -f data/policies/policy-app-access.yaml
[[ $? -eq 0 ]] && echo "Done!!!"  || echo "ERROR!!!"
